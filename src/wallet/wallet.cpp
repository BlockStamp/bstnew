...
bool CWallet::ChangeMessengerPassphrase(const SecureString& strOldWalletPassphrase, const SecureString& strNewWalletPassphrase)
{
    bool fWasLocked = IsMsgLocked();

    {
        ...
        CCrypter crypter;
        CKeyingMaterial _vMasterKey;
        for (MasterKeyMap::value_type& pMasterKey : mapMessengerMasterKeys)
        {
            if(!crypter.SetKeyFromPassphrase(strOldWalletPassphrase, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivationMethod))
                return false;
            if (!crypter.Decrypt(pMasterKey.second.vchCryptedKey, _vMasterKey))
            ...
            if (CCryptoKeyStore::MsgUnlock(_vMasterKey))
            {
                ...
                WalletLogPrintf("Messenger passphrase changed to an nDeriveIterations of %i\n", pMasterKey.second.nDeriveIterations);

                if (!crypter.SetKeyFromPassphrase(strNewWalletPassphrase, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivationMethod))
                    return false;
                if (!crypter.Encrypt(_vMasterKey, pMasterKey.second.vchCryptedKey))
                    return false;
                WalletBatch(*msgDatabase).WriteMessengerMasterKey(pMasterKey.first, pMasterKey.second);
                if (fWasLocked)
                    MsgLock();
                return true;
            }
...
void CWallet::AddEncrMsgToWalletIfNeeded(const CTransactionRef& ptx, const CBlockIndex* pIndex, int posInBlock) {
    ...
    if (IsFreeEncryptedMsg(opReturn) && internal_miner::verifyTransactionHash(*ptx, internal_miner::TxPoWCheck::FOR_DB)) {
        // modify op return
...
bool CWallet::SetMsgAddressBook(const std::string &address, const std::string &label)
{
    bool fUpdated = false;
    {
        LOCK(cs_wallet);
        std::map<std::string, std::string>::iterator mi = mapMessengerAddressBook.find(address);
        fUpdated = mi != mapMessengerAddressBook.end();
        mapMessengerAddressBook[address] = label;
    }

    NotifyMessengerAddressBookChanged(this, address, label, (fUpdated ? CT_UPDATED : CT_NEW));

    return WalletBatch(*msgDatabase).WriteMsgAddress(address, label);
}
